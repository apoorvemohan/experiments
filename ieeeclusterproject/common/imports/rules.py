...
#hyperthreading - switch ht on only if individual performance degrades after overall improvement
...
#hyperthreading - randomly switching to ht if overall performance improoves
def rule13(running_history_map, stats_history_map, app_stats_map1, threads, runninglist, total_passes):
        import copy
        import rules_utils as ru
        import multiprocessing as m
        import xml.etree.ElementTree as ET
...
#			if app in app_stats_map.keys():
	...
	if throughput > 1:
		next_runnable[0] = copy.deepcopy(running_history_map[total_passes][0])
		next_runnable[1] = copy.deepcopy(running_history_map[total_passes][1])
		probables_list = list(set(next_runnable[0]) - set(next_runnable[1]))
		while len(probables_list) > 0:
			app = random.choice(probables_list)
			tree = ET.parse(c.PARALLEL_DMTCP_APP_INSTANCE_DIR + '/' + app + '.xml')
			root = tree.getroot()
			app_core_req = int(root.findall('THREADS')[0].text)
			if (app not in running_history_map[total_passes][1]) and (app_core_req > 1):
				next_runnable[1].append(app)
				break
...
#ht - fix overcommit - flip ht for only jobs that work well under ht - mark job dirty if not performing well - so that it is not picked again for ht swithcing
def rule16(running_history_map, stats_history_map, app_stats_map1, threads, runninglist, total_passes):
        import copy
        import rules_utils as ru
        import multiprocessing as m
        import xml.etree.ElementTree as ET
...
#			if app in app_stats_map.keys():
	...
	if throughput > deg_ratio:
		...
		probables_list = []
		#filter jobs that work well with ht from this 
		for app in list(set(next_runnable[0]) - set(next_runnable[1])):
			if app_stats_map[app]['HT']:
				probables_list.append(app)

		while len(probables_list) > 0:
			app = random.choice(probables_list)
			if app not in running_history_map[total_passes][2]:
				tree = ET.parse(c.PARALLEL_DMTCP_APP_INSTANCE_DIR + '/' + app + '.xml')
				root = tree.getroot()
				app_core_req = int(root.findall('THREADS')[0].text)
				if (app not in running_history_map[total_passes][1]) and (app_core_req > 1):
		...
		next_runnable[0] = running_history_map[total_passes][0]
		next_runnable[1] = running_history_map[total_passes][1][:-1]
		next_runnable[2].append(running_history_map[total_passes][1][-1])
		return next_runnable
	else:
		while len(app_stats_map) > 0:
			#app = ru.get_app_with_max_threads(app_stats_map)
...
#ht - fix overcommit - flip ht for only jobs that work well under ht - mark job dirty if not performing well - so that it is not picked again for ht swithcing - if job finish after soft dealine
def rule18(running_history_map, stats_history_map, app_stats_map1, threads, runninglist, total_passes):
        import copy
        import rules_utils as ru
        import multiprocessing as m
        import xml.etree.ElementTree as ET
	...
	if threads == None:
		...
		for app in running_history_map[total_passes][0]:
			...
			thread_sum += int(root.findall('THREADS')[0].text)
		if thread_sum > totalthreads:
			deg_ratio += (((thread_sum - totalthreads)/100)*3)
		elif thread_sum < totalthreads:
			deg_ratio = 0.95
...
RULES = {'rule8': rule8, 'rule9' : rule9, 'rule10': rule10, 'rule11': rule11, 'rule13': rule13, 'rule16': rule16, 'rule15':rule15, 'rule14': rule14, 'rule18': rule18}
